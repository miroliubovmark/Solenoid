...
#define GetForce(MagneticMoment, dB, F)\
{\
    (F).m_f64X = (MagneticMoment).m_f64X * (dB).m_f64X;\
    (F).m_f64Y = (MagneticMoment).m_f64Y * (dB).m_f64Y;\
    (F).m_f64Z = (MagneticMoment).m_f64Z * (dB).m_f64Z;\
}
...
public:
    ...
    static BOOL RingOfCurrent_FieldDerivative(const CPoint3D& crRingCentrePoint, const F64 f64Rs, const F64 f64Current,
                                              const CPoint3D& crInvestigationPoint, CVector3D *pForce);
