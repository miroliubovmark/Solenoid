...
BOOL CIntegration::IntegrateSolenoid_Force(const Solenoid& crSolenoid, Ball& rBall, const CPoint3D& crInvestigationPoint, const std::vector<F64>& crVWireDensity, F64* pf64Fr_Result)
{
    *pf64Fr_Result = 0;
    ...
    for(U64 i = 0; i < crSolenoid.m_u64NSourcePoints; ++i)
    {
        ...
        /* Calculate Total current in the RingOfCurrent */
        f64Current = crSolenoid.m_f64Current * crVWireDensity[i] * fabs(f64Step);
